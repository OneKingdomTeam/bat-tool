...
add_action( 'wp_ajax_get_application_data', 'bat_get_application_data');

function bat_get_application_data(){

    $nonce_is_valid = wp_verify_nonce( $_POST['nonce'], 'bat_ajax' );
    ...
    if ( $user_is_logged_in && $nonce_is_valid !== false && $user_can_access ){
        ...
        // if all goes well it send the data
    ...
    wp_send_json( [ 'response' => 'unauthorised' ], 401 );
    wp_die();

};

add_action( 'wp_ajax_save_application_data', 'bat_save_application_data');


function bat_save_application_data(){
...
add_action( 'wp_ajax_get_recent_update_time', 'bat_get_recent_update_time');


function bat_get_recent_update_time(){
